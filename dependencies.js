...
{ constructor( rows, columns, angle )  
  { super( "positions", "normals", "texture_coords" );
    const circle_points = Array( rows ).fill( Vec.of( 1,0,0 ) )
                                       .map( (p,i,a) => Mat4.rotation( i/(a.length-1) * Math.PI - 0.5 * Math.PI , Vec.of( 0,1,0 ) )
                                                .times( p.to4(1) ).to3() );
    Surface_Of_Revolution.insert_transformed_copy_into( this, [ rows, columns, circle_points, , angle ] );         
  } }
